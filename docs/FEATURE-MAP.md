...
| Claude leaf routing | implemented | `.claude/agents/canuto-leaf.md`, `.agents/tools/claude-agent-mcp.py`, `.agents/tools/claude-architect.sh`, `.agents/tools/claude-reviewer.sh`, `.agents/hooks/install.sh` | Gives a Claude Maestro a native economical read-only leaf and lets a Codex Maestro call the equivalent `spawn_cheap_agent` through the existing Architect MCP; no third startup server |
...
| Desired-state hook reconciler | implemented | `.agents/hooks/managed-hooks*.json`, `.agents/hooks/reconcile-hooks.mjs` | Default install is plan-only; source-backed handlers and registration-only retirements use explicit manifests; apply requires the reviewed fingerprint; file/config verification, isolated rollback and external-entry preservation are enforced |
| Claude/Codex hook contract harness | implemented | `.agents/hooks/contracts/contract-harness.mjs`, `.agents/hooks/contracts/fixtures/` | Sanitized fixtures cover native payloads, responses, exit codes, subprocess failures, Gate intersection and non-blocking Advisory behavior without touching installed hooks |
