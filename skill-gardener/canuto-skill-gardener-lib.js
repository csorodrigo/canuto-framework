...
function sourceHistoryFiles(sourcePath, provider = '') {
  ...
  function walk(directory, depth) {
    ...
    for (const entry of listEntries(directory)) {
      ...
      else if (stat.isFile() && (file.endsWith('.jsonl') || (provider === 'hermes' && file.endsWith('.json')))) files.push(file);
    }
  }
  walk(allowlistedRoot, 0);
  return files.sort();
...
function isIgnorableMalformedRecordLine(line) {
  const text = String(line || '');
  if (/^[\0\s]*$/.test(text)) return true;
  return /"type"\s*:\s*"response_item"[\s\S]{0,4096}"payload"\s*:\s*\{[\s\S]{0,2048}"type"\s*:\s*"reasoning"/.test(text);
}

function readLocalSource({ source, cursor = {}, context, full = false }) {
  const files = sourceHistoryFiles(source.path, source.provider);
  ...
  for (const file of files) {
    ...
    for (const item of iterateBufferLines(buffer, start, completePrefixEnd)) {
      ...
      try { record = JSON.parse(line); } catch {
        if (isIgnorableMalformedRecordLine(line)) continue;
        partial = true; reason = 'schema-invalid'; break;
      }
