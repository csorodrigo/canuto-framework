...
Segunda leva da absorção edge-of-chaos, focada nas costuras que a auditoria 2026-08-01 mediu como ruidosas ou write-only:

- **Identidade em duas posturas**: leitura degrada e rotula (`canuto_project_slug`); escrita falha alto nomeando o gap (`canuto_require_project_slug`) com o critério "sobrevive a `git worktree add`?". `canuto_classify_vault` classifica o vault em `POPULATED`/`FRESH`/`ORPHAN`; `canuto_fresh_clone_check` detecta clone-do-template vs install vivo. ([ADR-0010](docs/adr/0010-identidade-duas-posturas-fail-loud.md))
- **Briefing garantido por hook**, com contrato por estágio: seção Identity é `required`; as demais são `expected-empty` com marcador honesto em 3 estados (vazio-honesto / vault declarado-e-ausente = quebrado / projeto fresco). `canuto_check_identity` valida com duas pernas (vault vivo + vault fresco descartável) para provar que o compositor não mente sobre vazio. Uso de memória é medido com ranqueia-antes-de-grava (`canuto_usage_record`/`canuto_usage_rerank`), nunca no caminho crítico. ([ADR-0011](docs/adr/0011-briefing-contrato-por-estagio.md))
- **Guard como auditor**: diff truncado pelo próprio guard nunca vira `HOLD` — vira `SKIP-TRUNCATED` advisory, registrado em metrics. Mecanismo que atacou a causa raiz dos overrides medidos (92 no mecesa, 246 no lucrando-ai). Inclui a escada `CANUTO_GUARD_MODE` (`0=off` / `1=observe` / `2=gate`, default gate — só a perna de review entra na escada), eventos de calibração com `mode`/`enforced` (`override-commit` com `reason` via `CANUTO_ALLOW_REASON`, `skip-truncated`, `guard-dark`) e a regra infra ≠ veredito: reviewer indisponível nunca vira `HOLD` nem `COMMIT` fabricado. ([ADR-0012](docs/adr/0012-guard-auditor-skip-truncated.md))
- **Delegação com fold de pendência**: pendência de delegação Codex é derivada do log a cada consulta (`delegation_ledger_pending`), nunca uma flag persistida. Só dois atos fecham um item — `delegation_dead_letter` (razão obrigatória) ou `delegation_declare_fallback` — e o Stop hook nomeia cada id pendente. ([ADR-0013](docs/adr/0013-delegacao-pendencia-e-fold.md))
- **Closeout atômico**: nota de sessão e evento `CLOSEOUT_PUBLISHED` são gravados no mesmo ato, com kernel de intenção (`intent`/`porque`/`proximo`) ou o contrato legado Summary/Proof/Next Entrypoint. Next-entrypoint acionável vira pending automático; admissão de retrabalho vira nota em `rework/`; pending/rework são endereçáveis (`status: proposed|set|dropped`, nunca deletados — dropar exige razão). ([ADR-0014](docs/adr/0014-closeout-atomico-pending-enderecavel.md))
...
| claude-architect / claude-reviewer | `~/.claude/scripts/claude-*.sh` (venv fixo; reserva `uv run --script`) | **so Codex** | Back-delegation Codex→Claude — nunca registrados no settings.json do Claude (numa sessao Claude seriam servidores mortos). Sobem no arranque de toda sessao Codex e custam ~2,6s cada (`import mcp` puxa pydantic); desligue com `.agents/tools/canuto-mcp.sh off` quando quiser abrir rapido |
